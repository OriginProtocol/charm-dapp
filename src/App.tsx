...
export function App() {
  ...
  const [ethereumContractBalance, setEthereumContractBalance] =
    useState<string>("0");
  const [arbitrumContractBalance, setArbitrumContractBalance] =
    useState<string>("0");
  const [tradeRate, setTradeRate] = useState<any>(null);

  const [ethereumWalletBalance, setEthereumWalletBalance] =
    useState<string>("0");
  const [arbitrumWalletBalance, setArbitrumWalletBalance] =
    useState<string>("0");
  ...
  const handleSwap = async (e) => {
    ...
    try {
      ...
      const transaction = prepareContractCall({
        contract,
        method:
          "function swapExactTokensForTokens(address inToken, address outToken, uint256 amountIn, uint256 amountOutMin, address to)",
  ...
  useEffect(() => {
    if (inputValue && exchangeRate) {
      ...
      const bigOutputValue = bigInputValue
        .mul(exchangeRate)
        ...
        <div className="flex justify-center mb-10 flex-col">
          ...
          >
            <div className="mb-4 input-container px-4 flex items-center">
              ...
              <div className="max-container w-12 mr-2">
                {arbitrumWalletBalance !== "0" &&
                  arbitrumWalletBalance !== inputValue && (
                    <a className="max text-yellow-400" onClick={handleMax}>
                      Max
                    </a>
                  )}
              </div>
            ...
            <button
              className="bg-blue-500 hover:bg-blue-700 text-white font-bold p-4 rounded focus:outline-none w-full"
              type="submit"
            >
              Swap
            </button>
          ...
          <div className="contracts flex justify-center mb-20 px-8 mx-auto">
            <div className="card p-4 mr-2 flex-grow text-center">
              ...
              <p>
                <strong>{df.format(ethereumContractBalance)} ETH</strong>
              </p>
            </div>
            <div className="card p-4 flex-grow text-center">
              <h3>Arbitrum liquidity</h3>
              <p>
                <strong>{df.format(arbitrumContractBalance)} ETH</strong>
              </p>
            </div>
          </div>
          <div className="pending-claims text-center mb-20">
            <div className="box p-4 border rounded-lg max-w-md mx-auto">
              <h4 className="text-xl font-bold mb-4">Pending Claims</h4>
...
function Navbar() {
  return (
    <nav className="flex justify-between">
      <div id="logo">
        <span className="translucent">CH</span>ARM
